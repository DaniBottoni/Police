...
async function saveWarning(key, data) {
    ...
    ).catch(e => console.error('saveWarning failed:', e.message));
}
async function deleteWarning(key) {
    activeWarnings.delete(key);
    pool.query('DELETE FROM warnings WHERE key = $1', [key]).catch(e => console.error('deleteWarning failed:', e.message));
}
...
async function addHistory(guildId, userId, entry) {
    ...
    ).catch(e => console.error('addHistory failed:', e.message));
}
...
async function addNote(guildId, userId, note) {
    ...
    ).catch(e => console.error('addNote failed:', e.message));
}
...
client.on('interactionCreate', async interaction => {
  try {
    ...
    // Defer only for commands that must await a DB read (history, note view)
    const deferCommands = ['history', 'note'];
    ...
    if (commandName === 'help') {
        ...
        await interaction.reply({ embeds: [helpOverview], components: [helpButtons, helpButtons2] });
    }
    ...
    // ── /config ────────────────────────────────────────────────────────────
        ...
        if (sub === 'access') {
            ...
            await saveConfig(guildId, config);
            await interaction.reply({ content: '✅ Mod-log channel removed.' });
        }
    }
    ...
    // ── /warn ──────────────────────────────────────────────────────────────
        ...
        try {
            ...
            console.error(error);
            await interaction.reply({ content: '❌ Failed to assign warning. Check permissions.' });
        }
    }
    ...
    // ── /timeout ───────────────────────────────────────────────────────────
        ...
        try {
            ...
            console.error(error);
            await interaction.reply({ content: '❌ Failed to apply timeout. Check permissions and role hierarchy.' });
        }
    }
    ...
    // ── /warnlist ──────────────────────────────────────────────────────────
        ...
        await interaction.reply({ embeds: [embed], components: warnlistRow(page, totalPages, guildId) });
    }
    ...
    // ── /kick ──────────────────────────────────────────────────────────────
        ...
        try {
            ...
            console.error(error);
            await interaction.reply({ content: '❌ Failed to kick user. Check permissions.' });
        }
    }
    ...
    // ── /ban ───────────────────────────────────────────────────────────────
        ...
        try {
            ...
            console.error(error);
            await interaction.reply({ content: '❌ Failed to ban user. Check permissions.' });
        }
    }
    ...
    // ── /escalation ────────────────────────────────────────────────────────
        ...
        if (sub === 'set') {
            ...
            delete esc.cap;
            await saveConfig(guildId, config);
            await interaction.reply({ content: '✅ Level cap removed.' });
