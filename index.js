...
client.on('messageCreate', async message => {
    ...
    for (const att of attachments) {
        ...
        if (shouldDelete) {
            if (canDelete) {
                message.delete().catch(e => console.error('scam: delete failed:', e.message));
            } else {
                console.error(`scam: cannot delete in ${message.channel.id} — missing ManageMessages`);
            }
        }
        let timedOut = false;
        ...
        message.channel.send({ embeds: [E2('#ff0000','Scam Image Detected').setDescription(`${message.author}'s message was removed — it matched a known scam image.`).addFields({ name: 'Matched', value: match.label, inline: true }, ...(timedOut ? [{ name: 'Consequence', value: `Timed out for ${timeoutDisplay}`, inline: true }] : []))] }).catch(() => {});
  ...
  try {
    ...
    if (commandName === 'invite') {
            ...
            try {
                ...
                if (delMsgDur) {
                    ...
                    for (const ch of channels.values()) {
                        try {
                            let fetched; let lastId;
                            do {
                                fetched = await ch.messages.fetch({ limit: 100, ...(lastId ? { before: lastId } : {}) });
                                const toDelete = fetched.filter(m => m.author.id === user.id && m.createdTimestamp >= since && Date.now() - m.createdTimestamp < 1_209_600_000);
                                if (toDelete.size) { await ch.bulkDelete(toDelete).catch(() => {}); deletedMsgCount += toDelete.size; }
        ...
        if (sub === 'delete') {
            ...
            const withinStr = interaction.options.getString('within');
            const withinDur = withinStr ? parseDuration(withinStr) : null;
            if (withinStr && (!withinDur || withinDur.isForever)) return reply('❌ Invalid within duration. Use `m:s`, `h:m:s`, or `d:h:m:s`.');
            if (!filterUser && !withinStr && !count) return reply('❌ Provide at least one filter: user, count, or within.');
            await interaction.deferReply({ flags: [MessageFlags.Ephemeral] });
            const since = withinDur ? Date.now() - withinDur.totalMs : null;
            const channel = interaction.channel;
            if (!botMember.permissionsIn(channel).has(PermissionFlagsBits.ManageMessages)) return reply('❌ I need ManageMessages in this channel.');
            ...
            try {
                outer: do {
                    ...
                    const eligible = fetched.filter(m => {
                        ...
                        if (since && m.createdTimestamp < since) return false;
                        if (Date.now() - m.createdTimestamp >= 1_209_600_000) return false; // >14 days, can't bulk delete
                        return true;
                    });
                    // Stop fetching if we've passed the time window
                    if (since && fetched.last()?.createdTimestamp < since) {
                        if (eligible.size) { await channel.bulkDelete(eligible).catch(() => {}); deleted += eligible.size; }
                        break;
                    }
                    const batch = [...eligible.values()].slice(0, count - deleted);
                    if (batch.length) { await channel.bulkDelete(batch).catch(() => {}); deleted += batch.length; }
                    if (deleted >= count) break outer;
                    lastId = fetched.last()?.id;
                } while (true);
            ...
            logMod(interaction.guild, guildId, E('#ff6600','Messages Deleted').addFields({ name: 'Channel', value: `${channel}`, inline: true }, { name: 'Deleted', value: `${deleted}`, inline: true }, ...(filterUser ? [{ name: 'User Filter', value: `${filterUser}`, inline: true }] : []), ...(withinDisplay ? [{ name: 'Within', value: withinDisplay, inline: true }] : []), { name: 'Moderator', value: `${interaction.user}`, inline: true }));
            await reply(`✅ Deleted **${deleted}** message${deleted !== 1 ? 's' : ''} in ${channel}.`);
        }
        else if (sub === 'purge') {
            const withinStr = interaction.options.getString('within');
            const filterUser = interaction.options.getUser('user');
            ...
            for (const ch of channels.values()) {
                ...
                try {
                    do {
                        ...
                        const eligible = fetched.filter(m => {
                            if (filterUser && m.author.id !== filterUser.id) return false;
                            if (m.createdTimestamp < since) return false;
                            if (Date.now() - m.createdTimestamp >= 1_209_600_000) return false;
                            return true;
                        });
                        if (fetched.last()?.createdTimestamp < since) {
                            if (eligible.size) { await ch.bulkDelete(eligible).catch(() => {}); chDeleted += eligible.size; }
                            break;
                        }
                        if (eligible.size) { await ch.bulkDelete(eligible).catch(() => {}); chDeleted += eligible.size; }
                        lastId = fetched.last()?.id;
                ...
                if (chDeleted > 0) { totalDeleted += chDeleted; channelsAffected++; }
            }
