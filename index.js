...
client.once('ready', async () => {
    ...
    const commands = [
        ...
        new SlashCommandBuilder().setName('config').setDescription('Configure the bot')
            ...
            .addSubcommand(s => s.setName('warndm').setDescription('Toggle warn DMs').addBooleanOption(o => o.setName('enabled').setDescription('Enable or disable').setRequired(true))),
...
client.on('messageCreate', async message => {
    ...
    for (const att of attachments) {
        ...
        const E2 = (c, t) => new EmbedBuilder().setColor(c).setTitle(t).setTimestamp();
        const cfg2 = await getConfig(guildId), logCh = cfg2.logChannelId && message.guild.channels.cache.get(cfg2.logChannelId);
  ...
  try {
    ...
    if (commandName === 'invite') {
        ...
        if (sub === 'access') { await showAccessControlConfig(interaction, guildId); }
            ...
            await reply({ embeds: [E(enabled ? '#00ff00' : '#FFA500', enabled ? 'Warn DMs Enabled' : 'Warn DMs Disabled').setDescription(enabled ? "Users will be DM'd when warned." : "Users will **not** be DM'd when warned.")], flags: [MessageFlags.Ephemeral] });
        }
    }
